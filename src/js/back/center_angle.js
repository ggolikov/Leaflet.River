...
// debugger;
            ...
            points.forEach(function(point){
                point.sigma = Math.sqrt(Math.pow((limits[1].projected.x - point[0]), 2) + Math.pow((limits[1].projected.y - point[1]), 2)) - (limits[1]._mRadius / Math.cos((Math.PI * limits[1].lat) / 180));
            })
            console.log(points);
